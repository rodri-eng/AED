...
inline int get_hash_value(const std::string &key, int m) {
  const int B = 311;
  const int MOD = 1e9 + 7;
  int hash_value = 0;
  for (char c : key) {
...
template <typename key_type, typename value_type> struct my_map {
  int m;
  int _size;
  std::vector<std::vector<std::pair<key_type, value_type>>> chains;
  ...
  void erase(const key_type &key) {
    ...
    if (at != (int)chains[chain_position].size()) {
      if (at + 1 < (int)chains[chain_position].size()) {
        std::swap(chains[chain_position][at], chains[chain_position].back());
      }
